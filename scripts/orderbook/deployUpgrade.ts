...
async function main() {
  ...
  console.log(
    `Upgrading orderbook with the account: ${owner.address} on chain id: ${(await ethers.provider.getNetwork()).chainId}`,
  );

  const timeout = 600 * 1000; // 10 minutes
  const SamWitchOrderBook = await ethers.getContractFactory("SamWitchOrderBook");
  const swob = (await upgrades.upgradeProxy(SAMWITCH_ORDERBOOK_ADDRESS, SamWitchOrderBook, {
    kind: "uups",
    timeout,
